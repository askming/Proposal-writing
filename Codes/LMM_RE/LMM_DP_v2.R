...
# load model files
setwd("/work/02784/myang3/proposal_sim_revision/model/LMM")
### 1. LMM using DPM prior ##########
model.file<-"/work/02784/myang3/proposal_sim_revision/model/LMM/model_dp2.txt"
...
runjags = function(data, y='y', x='x', model, I, K){
	...
	jags.data <- list(y=matrix(unlist(data[[y]]),nrow=I),x=array(unlist(data[[x]]),dim=c(I,J,2)),I=I,J=J,N=N)
...
t1 = Sys.time()
mn_t3_2re_new_30_NCDP2_lmm <- foreach(i=1:1,.packages=c("rjags","R2jags","foreach"),.verbose=T ) %dopar% {
  	set.seed(123)
  	temp <- runjags(mn_t3_2re_30_new[[i]], y='y', x='x', I=100, K=200, model=model.file)
  	temp
}

#setwd("/work/02784/myang3/proposal_sim_revision/data_out/BQLMM")
#save(mn_t3_2re_new_30_NCDP2,file="mn_t3_2re_new_30_NCDP2.Rdata")
t2 = Sys.time()
(t2-t1)
